...
\begin{grammar}
<function-declaration> ::= `def' <identifier> `(' <function-decl-params> `)' <statement> 
...
If the expression provided to the if statement evalutes to a non-zero value then the subsequent statement is executed.  If the expression evaluates to a value of zero, then the statement is not executed.
\begin{grammar}
